---
title: "HR Analytics: Prediction of Candidates Who Stay/Leave the Company and the City Development Index"
author: "Team 3: Xinping Yu, Ruchika Venkateswaran, Yigit, Bosoo, Muyan"
date: "2/22/2021"
output:
...
The charts below displays that candidates looking for job change belong to cities with lower development indexes. Candidates might be willing to leave the company after training in order to elocate to a different city that is more developed with greater scope for technological innovations. 
...
```{r}
 ggplot(df, aes(x = `city_development_index`, y = `education_level`, fill = ..x..)) +
   geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_fill_viridis(name = 'City_development_index', option = 'C') +
   labs(title = 'Education level by City index') +
   theme_ipsum() +
     theme(
...
ggplot(df, aes(city_development_index, fill = as.factor(target)))+
  geom_density(alpha = 0.5)+ ggtitle('Density Plot for City Development Index by Candidates who Stay/Leave the Company') + ylab('Density') + xlab('City Development Index')
```
...
hp + facet_grid(gender ~ education_level) + ggtitle('Experience by Gender and Graduate Level') + ylab('Number of Candidates') + xlab('Experience')
hp + facet_grid(education_level ~ enrolled_university)+ ggtitle('Experience by University and Graduate Level') + ylab('Number of Candidates') + xlab('Experience')
```

```{r}
#examining outliers 
ggplot(df, aes(x=relevent_experience, y=last_new_job, fill=enrolled_university)) + geom_boxplot()+ ggtitle('Distribution of Last New Job by Experience and University Enrollment') + ylab('Last New Job') + xlab('Relevant Experience')
```
...
ggplot(data = df) +
  geom_bar(mapping = aes(x = gender, fill = last_new_job), position = "dodge", fill="tomato3") + ggtitle('Gender count by last new job') + ylab('Number of Candidates') + xlab('Gender')
```
...
df %>% 
  ...
  geom_col()+
  coord_flip() + ggtitle('Number of Candidates Who Stay/Leave by Company Size') + ylab('Company Size') + xlab('Number of Candidates')
...
```
Based on the variable importance plot of our random forest model, we can observe that city development index, experience, and last new job are the most important features in predicting if a candidate will continue with a company. 
...
We can see from this overly simplified decision tree above that city development index, experience, and training hours are the top 3 variables that are important in predicting if a candidate will continue with our company or not.
...
The gradient boost model also identifies city development index as the top feature in predicting whether a candidate will stay or leave a company. Additional features that are important no university enrollment, experience, training hours, and last new job are most important.
...
We created a train dataset based on the target variable "city_development_index" to see which variable would impact "city_development_index" the most. We included the following four features (most important featurs from our classification models) in our new train and test data : "experience", "enrolled_university_enrollment", "last_new_job", and "major_disciplineStem''. By picking linear regression as our best model, we can easily see that "experience", "enrolled_university_enrollment", and "last_new_job" have positive relationships with the "city_development_index". And as "enrolled_university_enrollment" increases, the target variable "city_development_index" increases more compared with the other two variables which all have a positive relationship with "city_development_index". "major_disciplineStem '' has a negative relationship on the city development index. 

Therefore, when the company wants to predict the probability of a candidate continuing to work in the same company, it should first place emphasis on the "city_development_index", followed by "enrolled_university_enrollment", "experience", and "last_new_job". The company should invest more on training candidate from cities with higher development index, university enrollments and work experience because as the number becomes higher, they are more likely to stay in the company after training. In this way, the company will save costs and time spent on training employees who want to leave the firm.
